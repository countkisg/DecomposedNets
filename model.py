...
class InfoGAN(object):
    ...
    def generate(self, z_var, reuse=None):
        if self.network_type == 'mnist':
            ...
            with tf.variable_scope('g_net', reuse=reuse):
                noise_code = tf.reshape(z_var, [self.batch_size, self.latent_dist.dim])
                h0 = lrelu(linear(noise_code, 4*3*448, name='ge_face_linear0'))
                h1 = lrelu(self.ge_face_bn1(deconv2d(tf.reshape(h0, shape=[self.batch_size, 4, 3, 448]),
                                                     output_shape=[self.batch_size, 7, 6, 256], k_h=4, k_w=4,
                                                     d_h=2, d_w=2,
                                                     name='ge_face_deconv0')))
                h2 = lrelu(deconv2d(h1, output_shape=[self.batch_size, 14, 11, 128], k_h=4, k_w=4,
                                    name='ge_face_deconv1'))
                h3 = lrelu(deconv2d(h2, output_shape=[self.batch_size, 28, 22, 64], k_h=4, k_w=4,
                                    name='ge_face_deconv2'))
                x_dist_flat = deconv2d(h3, output_shape=[self.batch_size, 55, 44, 3], k_w=4, k_h=4,
